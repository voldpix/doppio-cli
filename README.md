...
The core unit is a `.dopo` file under `.doppio/recipes`. Doppio resolves requests by shorthand, so `.doppio/recipes/auth/login.dopo` can be executed as:
...
`doppio init` creates `.doppio/default.seed`, `.doppio/seeds/`, `.doppio/recipes/example.dopo`, and `.doppio/recipes/test.dopo`, then prints the full `.doppio` path as a tree.
...
```text
@var in the request file > selected .doppio/seeds/<name>.seed > .doppio/default.seed > OS environment
```

Create seed overlays with:
...
`doppio list` and `doppio ls` print the full `.doppio` path on top and then a recipe tree without exposing the internal `recipes` folder as part of the shorthand.
...
`doppio run` executes the request and prints a short brewing status, request details, response status, elapsed time, response headers, and response body. Non-2xx HTTP responses still print response details and exit with failure.

`--env <name>` is supported by `run`, `preview`, `check`, and `doctor`. It loads `.doppio/seeds/<name>.seed` after `default.seed` and before request-local `@var` values. `show`, `format`, `list`, `rm`, `mv`, `cp`, `rename`, `clean`, and `docs` do not need seed selection.

`doppio format` formats `.dopo` files. With no target it formats every request under `.doppio/recipes`. With a target it accepts the same project shorthand style, so `doppio format auth/login`, `doppio format auth/login.dopo`, and `doppio format auth` work inside a Doppio project.
...
`doppio doctor` inspects the current Doppio project and prints pass/warn/fail findings for project discovery, seed files, the recipes folder, request count, and no-network request validation. It is the fastest sanity check when a repo has been cloned or generated by an agent.

`--save` writes the rendered run report next to the resolved request file as `<request-name>-<epochMillis>.txt`. `doppio clean` removes generated report files under `.doppio/recipes`. `doppio rm` moves recipe files to `.doppio/trash` instead of deleting them outright.

`doppio mv`, `doppio cp`, and `doppio rename` organize recipe files under `.doppio/recipes`. They use the same shorthand and optional `.dopo` extension as `run`, create destination folders when needed, and refuse to overwrite existing files.
...
On startup it prints the resolved `.doppio` path and then prompts as `doppio:[default]>` or `doppio:[dev]>`. The prompt, seed state, edit paths, and run result are colorized unless `NO_COLOR` is set. Shell history is stored in `~/.config/doppio/history`. Recent project paths are stored in `~/.config/doppio/status.json` so running `doppio shell` outside a project can offer a picker. If no project can be found it exits with:

```text
No Doppio project found. Navigate to a Doppio project or run doppio init first.
```

Request commands accept the same shorthand as the CLI. Stem lookup also works, so `run login` can find `.doppio/recipes/auth/login.dopo`. If more than one request matches, the shell asks you to choose.
...
doppio:[default]> mv auth/login auth/session/login
doppio:[default]> cp auth/session/login auth/session/login-copy
doppio:[default]> rename auth/session/login-copy login-v2
doppio:[default]> rm auth/login
```

Seed helpers are available inside the same session:
...
doppio:[default]> seed use dev
doppio:[dev]> run login
doppio:[dev]> seed clear
```

Config helpers let you configure one terminal workflow once:

```text
doppio:[default]> config editor show
doppio:[default]> config editor use nano
doppio:[default]> config editor use "code -w"
doppio:[default]> config editor clear
```
...
`doppio gen` creates editable request placeholders under `.doppio/recipes`.
