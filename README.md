...
`--env <name>` is supported by `run`, `preview`, `check`, and `doctor`. It loads `.doppio/seeds/<name>.seed` after `default.seed` and before request-local `@var` values. `show`, `format`, `list`, `rm`, `clean`, and `docs` do not need seed selection.
